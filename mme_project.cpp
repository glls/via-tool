...
//  global functions
//---------------------------------------------------------------------------
int open_mme(const char *path, MME_PROJECT *project, VF_PluginFunc *func, VF_PluginFuncEditExt *func_edit_ext, int lang);
void close_mme(MME_PROJECT *project);

int output_mme(MME_PROJECT *project, char *out_path, HRESULT (_stdcall *callback)(char *path, DWORD percent));

const char *get_error_msg_mme(MME_PROJECT *project);
const char *get_status_str_mme(MME_PROJECT *project);

int is_open_mme(MME_PROJECT *project);

__int64 set_frame_index_mme(MME_PROJECT *project, __int64 frame_index);
__int64 get_frame_index_mme(MME_PROJECT *project);
__int64 go_out_point_mme(MME_PROJECT *project);
__int64 go_in_point_mme(MME_PROJECT *project);
__int64 go_start_frame_mme(MME_PROJECT *project);
__int64 go_end_frame_mme(MME_PROJECT *project);
__int64 go_next_i_frame_mme(MME_PROJECT *project);
__int64 go_prev_i_frame_mme(MME_PROJECT *project);

int set_in_point_mme(MME_PROJECT *project);
int set_out_point_mme(MME_PROJECT *project);

const VOID *get_frame_mme(MME_PROJECT *project);
const BITMAPINFOHEADER *get_dib_header_mme(MME_PROJECT *project);

int get_frame_width_mme(MME_PROJECT *project);
int get_frame_height_mme(MME_PROJECT *project);

__int64 get_frame_count_mme(MME_PROJECT *project);
__int64 get_in_point_mme(MME_PROJECT *project);
__int64 get_out_point_mme(MME_PROJECT *project);
...
//---------------------------------------------------------------------------
int open_mme(const char *path, MME_PROJECT *project, VF_PluginFunc *func,
	VF_PluginFuncEditExt *func_edit_ext, int lang)
{
	int size;
...
int output_mme(MME_PROJECT *project, char *out_path, HRESULT (_stdcall *callback)(char *path, DWORD percent))
{
	...
	if(project->func_edit_ext->Cut(project->file, &opt) == VF_ERROR){
		sprintf(project->error_msg, msg_table[project->lang][MSG_FAILED_TO_STORE_FILE], out_path); // TODO: check 
		return 0;
	}
...
__int64 go_out_point_mme(MME_PROJECT *project)
{
	return set_frame_index_mme(project, project->out_point-1);
}
//---------------------------------------------------------------------------

__int64 go_in_point_mme(MME_PROJECT *project)
{
	return set_frame_index_mme(project, project->in_point);
}
//---------------------------------------------------------------------------

__int64 go_start_frame_mme(MME_PROJECT *project)
{
	return set_frame_index_mme(project, 0);
}
//---------------------------------------------------------------------------
...
__int64 go_end_frame_mme(MME_PROJECT *project)
{
	...
	return set_frame_index_mme(project, max-1);
}
//---------------------------------------------------------------------------
...
__int64 go_next_i_frame_mme(MME_PROJECT *project)
{
	...
	for(i=project->frame_index+1;i<max;i++){
		...
		if(frame_type.dwFrameType == (VF_FRAME_TYPE_STARTABLE|VF_FRAME_TYPE_ENDABLE)){
			return set_frame_index_mme(project, i);
		}
	}
...
static void set_status_str(MME_PROJECT *project)
{
  ...
  */
  ...
  strcpy(project->status_str, "frame type : ");
	project->func_edit_ext->GetFrameType(project->file, &frame_type);
